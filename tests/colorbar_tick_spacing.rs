...
fn hexbin_with_peak(peak_count: usize) -> Vec<Plot> {
...
#[test]
fn data_max_label_suppressed_when_adjacent_to_decade() {
    // peak 112 → displayed max = 111, one notch above the 100 decade tick.
    let plots = hexbin_with_peak(112);
...
#[test]
fn data_max_label_kept_when_clear_of_decade() {
    // peak 300 → displayed max = 299, ~half a decade above 100: no overprint.
    let plots = hexbin_with_peak(300);
